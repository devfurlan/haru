...
- **Telefones**: armazenados no banco em E.164 sem máscara (ex.: `5511912345678`). Na UI exibida ao usuário final - em TODOS os apps - SEMPRE formatar para humano BR: `(11) 91234-5678`. Use `formatPhoneBR` de [apps/web/src/lib/format.ts](apps/web/src/lib/format.ts) / [apps/bot/src/lib/format.ts](apps/bot/src/lib/format.ts). Exceções (mantêm bruto): valores consumidos por máquina (payloads de webhook, links `wa.me/`, recipient de envio).
- **Templates da Meta (WhatsApp)**: a lista canônica de templates pré-aprovados (nome default, idioma, parâmetros `{{1}}`…, onde é enviado) vive na seção "Templates da Meta (WhatsApp)" do [README.md](README.md). SEMPRE que adicionar ou alterar um template (novo evento, mudança no número/ordem dos parâmetros, ou no default do nome), atualizar aquela tabela na MESMA alteração. Os parâmetros enviados pelo código têm que bater exatamente com o template aprovado na Meta - divergência faz a Meta recusar o envio silenciosamente.
- **Modais com ação não fecham por fora**: todo modal/dialog que contém formulário ou ação (criar, editar, remarcar, adicionar, convidar, etc.) NÃO pode fechar ao clicar no overlay nem com ESC - só pelo X ou por um botão "Cancelar" interno. Isso evita perda acidental do que foi digitado. No código: passe `dismissable={false}` ao `DialogContent` de [apps/web/src/components/ui/dialog.tsx](apps/web/src/components/ui/dialog.tsx) (default é `true`). Modais que são apenas seletores/visualização (date-picker, preview, confirmação simples sem campos) podem manter o fechamento por overlay.
