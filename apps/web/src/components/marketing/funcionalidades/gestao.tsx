export function Gestao() {
  return (
    <section id="gestao" className="scroll-mt-19.5">
      <div className="mx-auto max-w-[1200px] px-[clamp(20px,5vw,40px)] py-[clamp(40px,6vw,72px)]">
        <div className="grid grid-cols-[repeat(auto-fit,minmax(320px,1fr))] items-center gap-[clamp(30px,5vw,64px)]">
          <div className="border-line bg-cream mx-auto flex w-full max-w-[480px] overflow-hidden rounded-[18px] border shadow-[var(--shadow-card)]">
            <div className="bg-green-deep w-15 flex flex-none flex-col items-center gap-2 py-3.5">
              <div className="bg-green-bright text-green-deep grid h-8 w-8 place-items-center rounded-[10px] font-serif text-[16px] font-semibold leading-[normal]">
                d
              </div>
              <div className="h-1.5"></div>
              <div className="bg-green-bright h-8.5 w-8.5 grid place-items-center rounded-[10px]">
                <svg
                  width="17"
                  height="17"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="var(--emerald)"
                  strokeWidth="2.1"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
                  <rect x="3" y="3" width="7.5" height="7.5" rx="2" />
                  <rect x="13.5" y="3" width="7.5" height="7.5" rx="2" />
                  <rect x="3" y="13.5" width="7.5" height="7.5" rx="2" />
                  <rect x="13.5" y="13.5" width="7.5" height="7.5" rx="2" />
                </svg>
              </div>
              <div className="h-8.5 w-8.5 grid place-items-center rounded-[10px]">
                <svg
                  width="17"
                  height="17"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="var(--on-emerald-mut)"
                  strokeWidth="2.1"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
                  <rect x="3" y="5" width="18" height="16" rx="3" />
                  <path d="M3 10h18M8 3v4M16 3v4" />
                </svg>
              </div>
              <div className="h-8.5 w-8.5 grid place-items-center rounded-[10px]">
                <svg
                  width="17"
                  height="17"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="var(--on-emerald-mut)"
                  strokeWidth="2.1"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
                  <circle cx="12" cy="8" r="3.5" />
                  <path d="M5 20c.7-3.8 3.4-6 7-6s6.3 2.2 7 6" />
                </svg>
              </div>
              <div className="h-8.5 w-8.5 grid place-items-center rounded-[10px]">
                <svg
                  width="17"
                  height="17"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="var(--on-emerald-mut)"
                  strokeWidth="2.1"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
                  <circle cx="12" cy="12" r="9" />
                  <path d="M12 7v5l3 2" />
                </svg>
              </div>
            </div>
            <div className="px-4.5 min-w-0 flex-1 py-4">
              <div className="flex items-baseline justify-between">
                <span className="text-ink font-serif text-[17px] font-medium leading-[normal]">
                  Início
                </span>
                <span className="text-ink-50 font-sans text-[11.5px] font-medium leading-[normal]">
                  Sáb, 5 jul
                </span>
              </div>
              <div className="mt-3.5 grid grid-cols-[1fr_1fr] gap-2">
                <div className="border-line bg-paper rounded-[12px] border px-3 py-3">
                  <div className="text-ink-50 font-sans text-[9px] font-bold uppercase leading-[normal] tracking-[.08em]">
                    Faturamento
                  </div>
                  <div className="text-ink mt-1 font-serif text-[18px] font-semibold leading-[normal]">
                    R$ 3.240
                  </div>
                  <div className="font-sans text-[10px] font-semibold leading-[normal] text-[#0C7E41]">
                    +12% na semana
                  </div>
                </div>
                <div className="border-line bg-paper rounded-[12px] border px-3 py-3">
                  <div className="text-ink-50 font-sans text-[9px] font-bold uppercase leading-[normal] tracking-[.08em]">
                    Agendamentos
                  </div>
                  <div className="text-ink mt-1 font-serif text-[18px] font-semibold leading-[normal]">
                    128
                  </div>
                  <div className="text-ink-50 font-sans text-[10px] font-semibold leading-[normal]">
                    esta semana
                  </div>
                </div>
                <div className="border-line bg-paper rounded-[12px] border px-3 py-3">
                  <div className="text-ink-50 font-sans text-[9px] font-bold uppercase leading-[normal] tracking-[.08em]">
                    Novos clientes
                  </div>
                  <div className="text-ink mt-1 font-serif text-[18px] font-semibold leading-[normal]">
                    34
                  </div>
                  <div className="font-sans text-[10px] font-semibold leading-[normal] text-[#0C7E41]">
                    +6 hoje
                  </div>
                </div>
                <div className="border-line bg-paper rounded-[12px] border px-3 py-3">
                  <div className="text-ink-50 font-sans text-[9px] font-bold uppercase leading-[normal] tracking-[.08em]">
                    Ocupação
                  </div>
                  <div className="text-ink mt-1 font-serif text-[18px] font-semibold leading-[normal]">
                    86%
                  </div>
                  <div className="text-ink-50 font-sans text-[10px] font-semibold leading-[normal]">
                    das cadeiras
                  </div>
                </div>
              </div>
              <div className="bg-coral-tint mt-2.5 flex items-center gap-2.5 rounded-[12px] px-3 py-2.5">
                <span className="bg-coral h-2 w-2 flex-none rounded-[50%]"></span>
                <span className="text-ink min-w-0 flex-1 font-sans text-[12px] font-semibold leading-[normal]">
                  3 clientes querem falar com você
                </span>
                <span className="text-coral font-sans text-[11px] font-bold leading-[normal]">
                  abrir
                </span>
              </div>
            </div>
          </div>
          <div>
            <div className="mb-3.5 inline-flex items-center gap-2">
              <span className="bg-coral h-0.5 w-5 rounded-[2px]"></span>
              <span className="font-sans text-[11px] font-bold uppercase leading-[normal] tracking-[.16em] text-[#0C7E41]">
                Pilar 04 · Gestão
              </span>
            </div>
            <h2 className="text-green-deep mb-3.5 max-w-[460px] font-serif text-[clamp(26px,3.6vw,38px)] font-normal leading-[1.12] tracking-[-.02em]">
              O negócio inteiro numa <span className="italic text-[#0C7E41]">tela só</span>.
            </h2>
            <p className="text-ink-70 mb-5.5 max-w-[460px] font-sans text-[16.5px] font-normal leading-[1.6]">
              Agenda, conversas, clientes, serviços e relatórios no mesmo painel. Você abre de manhã
              e entende o dia num relance - no computador ou no celular.
            </p>
            <div className="flex max-w-[440px] flex-col gap-0.5">
              <div className="flex items-start gap-3 py-2">
                <svg
                  width="18"
                  height="18"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#2FD37A"
                  strokeWidth="2.6"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="mt-0.5 flex-none"
                >
                  <polyline points="20 6 9 17 4 12" />
                </svg>
                <span className="text-ink-70 font-sans text-[15.5px] font-normal leading-[1.5]">
                  Painel único, no computador e no celular
                </span>
              </div>
              <div className="flex items-start gap-3 py-2">
                <svg
                  width="18"
                  height="18"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#2FD37A"
                  strokeWidth="2.6"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="mt-0.5 flex-none"
                >
                  <polyline points="20 6 9 17 4 12" />
                </svg>
                <span className="text-ink-70 font-sans text-[15.5px] font-normal leading-[1.5]">
                  Conversas e atendimento num lugar só
                </span>
              </div>
              <div className="flex items-start gap-3 py-2">
                <svg
                  width="18"
                  height="18"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#2FD37A"
                  strokeWidth="2.6"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="mt-0.5 flex-none"
                >
                  <polyline points="20 6 9 17 4 12" />
                </svg>
                <span className="text-ink-70 font-sans text-[15.5px] font-normal leading-[1.5]">
                  Base de clientes com histórico completo
                </span>
              </div>
              <div className="flex items-start gap-3 py-2">
                <svg
                  width="18"
                  height="18"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="#2FD37A"
                  strokeWidth="2.6"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="mt-0.5 flex-none"
                >
                  <polyline points="20 6 9 17 4 12" />
                </svg>
                <span className="text-ink-70 font-sans text-[15.5px] font-normal leading-[1.5]">
                  Relatórios de faturamento e retenção
                </span>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
