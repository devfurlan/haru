...
export async function POST(req: Request) {
  ...
  const sub = parsed.asaasSubscriptionId
    ...
    : installmentRef
      ...
      : null;
  // Não achou (outro ambiente/sandbox, ou evento sem cobrança de assinatura) → 200.
  if (!sub) {
    return new Response('OK', { status: 200 });
  }
  ...
  if (parsed.effect === 'ACTIVE') {
    ...
    if (sub.pendingPlanTier) {
      const pendingPlan = await prisma.plan.findUnique({ where: { tier: sub.pendingPlanTier } });
      if (pendingPlan) {
        planChange = {
