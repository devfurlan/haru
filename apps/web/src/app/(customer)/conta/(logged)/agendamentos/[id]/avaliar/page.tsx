...
// Só atendimento CONCLUÍDO pode ser avaliado. O gate de ownership é o próprio
// customerAccountId (o id vem da URL, mas o atendimento tem que estar na lista do cliente);
// o gate real da escrita continua no server (upsertReview exige um COMPLETED no tenant).
export default async function AvaliarPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!item) notFound();
  if (item.status !== 'COMPLETED') redirect(`/conta/agendamentos/${id}`);
