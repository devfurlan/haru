...
assert.equal(snapshotPlan(plan, 'MONTHLY').priceCents, 6900);
assert.equal(snapshotPlan(plan, 'ANNUAL').priceCents, 69000);
assert.equal(snapshotPlan(plan, 'ANNUAL_INSTALLMENTS').priceCents, 6115);
assert.equal(snapshotPlan(plan, 'MONTHLY').appointmentsLimit, 250);
assert.equal(snapshotPlan(plan, 'MONTHLY').aiMessagesLimit, null); // base sem IA
...
assert.equal(snap.priceCents, 6900);
